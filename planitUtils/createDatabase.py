...
import copy
import Image
# from getPosition import getObjectPosition
import itertools
from dbFns.main import getObjectPosition,insertTrajectory,getTrajectory
...
def main(objList,env_colladafile,numSampleTraj=2):
	...
	permutations=len(objList)*(len(objList)-1)
	counter=1
	for r in itertools.product(objList, objList):
		if not r[0]==r[1]:
			print 'Executing Example:',counter,'/',permutations
			start_config=getObjectPosition({'envName':paramsFilePrepend,'objectName':r[0]})['objectPosition']
			end_config=getObjectPosition({'envName':paramsFilePrepend,'objectName':r[1]})['objectPosition']
			list_traj,env = planmanytraj(env,start_config,end_config,numPoints=numSampleTraj)
			insertTrajectory({'envName':paramsFilePrepend,'objectFrom':r[0],'objectTo':r[1],'trajectory':list_traj})
...
def playDbTraj(objList,env_colladafile):
	...
	for r in itertools.product(objList, objList):
		if not r[0]==r[1]:
			print 'Playing Example:',counter,'/',permutations
			print 'moving from ',r[0],'to',r[1]
			envName=(((env_colladafile.split('/'))[-1]).split('.'))[0]
			trajectory=getTrajectory({'envName':envName,'objectFrom':r[0],'objectTo':r[1]})['trajectory']
			Playtraj(trajectory,env)
			counter=counter+1
	env.Destroy()
